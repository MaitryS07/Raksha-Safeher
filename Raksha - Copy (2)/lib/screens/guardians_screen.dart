...
class _GuardiansScreenState extends State<GuardiansScreen> {
  ...
  Future<bool> _syncGuardiansToBackend() async {
    ...
    debugPrint("DEBUG: Syncing ${phones.length} guardians to backend: $phones");
    ...
    for (final phone in phones) {
      ...
      if (result["success"] != true) {
        debugPrint("DEBUG: Failed to sync guardian $phone → ${result["error"] ?? result["response"]}");
        allSuccess = false;
      }
    }
  ...
  Future<void> addGuardian() async {
    ...
    if (result["success"] == true) {
      ...
      debugPrint("SUCCESS: Guardian $formattedPhone added to backend");
    } else {
      Utils.showSnackBar(
        context,
        "Added locally, but sync failed: ${result["error"] ?? 'Unknown error'}",
      );
      debugPrint("ERROR: Failed to add $formattedPhone to backend → ${result["error"]}");
    }
  ...
  Future<void> deleteGuardian(int index) async {
    ...
    if (!success) {
      debugPrint("WARNING: Delete sync failed – some guardians may still be on server");
    }
  }
